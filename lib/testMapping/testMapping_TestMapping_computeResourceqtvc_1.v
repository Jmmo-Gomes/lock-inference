...
(* Why3 assumption *)
Definition setIsParameter (res:resource) (isParameter1:Init.Datatypes.bool) :
    resource :=
  resource'mk (position res) (ty res) (vis res) (nature res) (whatIs res)
  (guardedBy res) isParameter1.

Axiom setIsParameter'spec :
  forall (res:resource) (isParameter1:Init.Datatypes.bool),
  ((isParameter (setIsParameter res isParameter1)) = isParameter1).

Parameter isField: Numbers.BinNums.Z -> Init.Datatypes.bool.

Axiom isField'def :
  forall (w:Numbers.BinNums.Z),
  ((w = 0%Z) -> ((isField w) = Init.Datatypes.true)) /\
  (~ (w = 0%Z) -> ((isField w) = Init.Datatypes.false)).

Axiom isField'spec :
  forall (w:Numbers.BinNums.Z), ((isField w) = Init.Datatypes.true) ->
  (w = 0%Z).
...
Parameter resourceAccessCons:
  Init.Datatypes.list Numbers.BinNums.Z -> resourceAccess.

Axiom resourceAccessCons'spec :
  forall (l:Init.Datatypes.list Numbers.BinNums.Z),
  ((list.Length.length l) = 5%Z) ->
  match l with
  | Init.Datatypes.cons lfirst
    (Init.Datatypes.cons lfirstRead
     (Init.Datatypes.cons llastRead
      (Init.Datatypes.cons lfirstWrite
       (Init.Datatypes.cons llastWrite Init.Datatypes.nil)))) =>
      ((first (resourceAccessCons l)) = lfirst) /\
      ((firstRead (resourceAccessCons l)) = lfirstRead) /\
      ((lastRead (resourceAccessCons l)) = llastRead) /\
      ((firstWrite (resourceAccessCons l)) = lfirstWrite) /\
      ((lastWrite (resourceAccessCons l)) = llastWrite)
  | _ => False
  end.
...
Parameter firstElement:
  forall {xi:Type} {xi_WT:WhyType xi}, Init.Datatypes.list xi -> xi.

Axiom firstElement'spec :
  forall {xi:Type} {xi_WT:WhyType xi},
  forall (list:Init.Datatypes.list xi), ~ (list = Init.Datatypes.nil) ->
  match list with
  | Init.Datatypes.nil => False
  | Init.Datatypes.cons x _ => ((firstElement list) = x)
  end.

Parameter tryRes:
  t5 resourceAccess -> Numbers.BinNums.Z ->
  Init.Datatypes.list Numbers.BinNums.Z -> t5 resource -> (t5 resourceAccess*
  t5 resource)%type.

Axiom tryRes'spec :
  forall (retMet:t5 resourceAccess) (resId:Numbers.BinNums.Z)
    (listLabels:Init.Datatypes.list Numbers.BinNums.Z)
    (resourceMap:t5 resource),
  ~ (listLabels = Init.Datatypes.nil) ->
  ((list.Length.length listLabels) = 5%Z) -> (0%Z <= resId)%Z ->
  match tryRes retMet resId listLabels resourceMap with
  | (newRaMap, newResourceMap) =>
      ((firstElement listLabels) = (-1%Z)%Z) ->
      (exists newResource:resource,
       (((mem2 resId resourceMap) = Init.Datatypes.true) ->
        (newResource = (view2 newResourceMap resId)) /\
        ((mem2 resId newResourceMap) = Init.Datatypes.true) /\
        ((isParameter newResource) = Init.Datatypes.true)) /\
       (~ ((mem2 resId resourceMap) = Init.Datatypes.true) ->
        ((mem2 resId newResourceMap) = Init.Datatypes.false)))
  end /\
  match tryRes retMet resId listLabels resourceMap with
  | (newRaMap, newResourceMap) =>
      exists rAccess:resourceAccess,
      (rAccess = (resourceAccessCons listLabels)) /\
      (rAccess = (view2 newRaMap resId))
  end.

Axiom xi : Type.
Parameter xi_WhyType : WhyType xi.
Existing Instance xi_WhyType.

Parameter resId: Numbers.BinNums.Z.

Parameter typ: Numbers.BinNums.Z.

Parameter visib: Numbers.BinNums.Z.

Parameter nature1: Init.Datatypes.bool.

Parameter whatIs1: Numbers.BinNums.Z.

Parameter resourceMap: t5 resource.

Parameter resourceMethodsMap: t5 (Init.Datatypes.list xi).

Axiom H : (0%Z <= resId)%Z.

Parameter newResourceMap: t5 resource.

Axiom Ensures :
  ((dom newResourceMap) = (set.Fset.add resId (dom resourceMap))).

Axiom Ensures1 :
  ((view2 newResourceMap) =
   (map.Map.set (view2 resourceMap) resId
    (resource'mk resId typ visib nature1 whatIs1 resId Init.Datatypes.false))).

Parameter newResourceMethodsMap: t5 (Init.Datatypes.list xi).

Axiom Ensures2 :
  ((dom newResourceMethodsMap) =
   (set.Fset.add resId (dom resourceMethodsMap))).

Axiom Ensures3 :
  ((view2 newResourceMethodsMap) =
   (map.Map.set (view2 resourceMethodsMap) resId Init.Datatypes.nil)).

(* Why3 goal *)
Theorem computeResource'vc :
  ((mem2 resId resourceMap) = Init.Datatypes.true).
Proof.
