...
let rec findGroups (roMap:resourceGroup list IntMap.t) keys lastlock= 
  match keys with
  | [] -> roMap
  | x :: xs ->
    ...
    if x < lastlock then 
      ...
      let rec iterateSet (set:resourceGroup list) roMap key= 
        match set with 
        | [] -> roMap 
        | x :: xs -> 
          let rGroup = x in 
...
let rec getLastLock (ropList:rOp list) lastlock (key:int)= 
    match ropList with 
    | [] -> lastlock
    | x :: xs -> 
      let opCode = x.op in
