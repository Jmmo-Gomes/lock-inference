...
open ResourceGroup
  ...
  (* Tests if the key is still in the map after being removed*)
 ...
 (*let rec checkSBE (set:ResourceGroup.resourceGroup list) resource2 label roMap foundSBE finished= 
    ...
    | x :: xs -> 
      ...
      if sbe && finished = false then
        begin
          let (rOp:rOp) = { op = 3; r = resource2} in 
          let newRoMap = remove label x rOp roMap in 
          let newFoundSBE = true in
          let newFinished = true in
          checkSBE xs resource2 label newRoMap newFoundSBE newFinished
        end
